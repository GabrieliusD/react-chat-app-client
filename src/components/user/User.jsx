...
import "./user.css";
export default function User({ userFound, clickHandle, icon }) {
  const profileImageLink = () => {
    if (userFound.image)
      return `${process.env.REACT_APP_API_URL}/images/${userFound.image}`;
  ...
  return (
    <div className="user-container" onClick={clickHandle}>
      <img className="user-img" src={profileImageLink()} alt="" />
